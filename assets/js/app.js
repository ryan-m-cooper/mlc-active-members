var config = {
  geojson: "data/mlc_active_membership_201608.geojson",
  title: "MLC Active Membeship",
  layerName: "Members",
  hoverProperty: "salutation2",
  sortProperty: "salutation2",
  sortOrder: "asc"
};
...
function drawCharts() {
  // City
  $(function() {
    var result = alasql("SELECT city AS label, COUNT(*) AS total FROM ? GROUP BY city", [features]);
    var columns = $.map(result, function(city) {
      return [[city.label, city.total]];
    });
...
var HERE_hybridDay = L.tileLayer('http://{s}.{base}.maps.cit.api.here.com/maptile/2.1/{type}/{mapID}/hybrid.day/{z}/{x}/{y}/{size}/{format}?app_id={app_id}&app_code={app_code}&lg={language}', {
  ...
  subdomains: '1234',
  mapID: 'newest',
  app_id: 'ZwgYSOUvjCMfVTP51C2F',
  app_code: 'efHfRedxYrIw-kEugfFYBQ',
  base: 'aerial',
  maxZoom: 20,
  type: 'maptile',
  language: 'eng',
...
var map = L.map("map", {
  layers: [OpenMapSurfer_Roads, featureLayer, highlightLayer]
}).fitWorld();
...
var baseLayers = {
  "Street Map": OpenMapSurfer_Roads,
  "Aerial Imagery": HERE_hybridDay
};
var overlayLayers = {
